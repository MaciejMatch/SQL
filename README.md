...
Between the values, first insert a comma followed by a space, and then a single space
...
and DepartmentID is equal to 6 or 4.
```sql
...
From the HumanResources.EmployeePayHistory table, select the columns: BusinessEntityID,Rate,RateChangeDate
Filter only those records where:

Rate is between 60 and 100
and the rate change occurred no earlier than the year 2009.
```sql
Select BusinessEntityID,Rate,RateChangeDate from HumanResources.EmployeePayHistory
where Rate between 60 and 100
and RateChangeDate>='20090101';
```
...
From the Person.CountryRegion table:
...
Join the following tables:Person.BusinessEntityAddress, Person.Address, Person.AddressType
...
Join the tables: HumanResources.Employee, HumanResources.JobCandidate
